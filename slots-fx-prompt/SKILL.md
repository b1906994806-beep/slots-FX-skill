...
|---|------|------|
| 1 | STEP 1.1 | 目标工具选定 |
| 2 | STEP 1.2 | 生成模式 + 意图（首尾帧时）确认 |
| 3 | STEP 2 | 场景类型选定 |
| 4 | STEP 3A-1 | 图标类型（Scene A） |
| 5 | STEP 3A-2 | 动效状态（Scene A） |
| 6 | STEP 3A-3b | 素材类型识别（置信度≥90%自动通过） |
| 7 | STEP 3B-1 | 弹窗/转场阶段（Scene B） |
| 8 | STEP 3B-2 | 转场方案（阶段[3]或[6]时，Scene B） |
| 9 | STEP 3B-3b | 素材类型识别（Scene B） |
| 10 | STEP 3C-1 | 播放模式（Scene C） |
| 11 | STEP 3C-2b | 素材类型识别（Scene C） |
| 12 | STEP 4 | 方案预览已输出，用户已选定方案 |
