...
**Seedance 2.0 模板：**
```
@图片1的[背景核心视觉特征]，[弹窗背景 / 加载页背景]动效，固定镜头，[风格质感]，轻柔大气。
...
- **Seedance 2.0：** 必须以`@图片1的[核心特征]，`开头（特征类型随素材而定：完整图标/纯主体→主体核心特征，纯文字→文字核心特征，纯背景→背景核心视觉特征）；意图[D]同时标注`@图片1`和`@图片2`；连贯叙述，不用`【】`标签；5–10行
